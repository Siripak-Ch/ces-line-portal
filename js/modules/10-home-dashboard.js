...
function setHomeFilter(team) {
    ...
    ['All','MED','LAB','EHS','TES'].forEach(t => {
        ...
        if(btn) btn.className = (t === team) 
            ? "px-3 py-1.5 rounded-lg text-xs font-bold text-white bg-indigo-600 shadow-lg transform scale-105 transition-all" 
        ...
        chartsCache[trendId] = new Chart(ctxT, {
            ...
            options: {
                ...
                scales: {
                    ...
                    y: { stacked: false, beginAtZero: true, suggestedMax: Math.ceil(maxVal * 1.15), grid: { color: '#f3f4f6' }, ticks: { font: { size: 10 } } }
                },
                plugins: {
                    legend: {
                        position: 'bottom',
...
function renderYearlyOTView() {
    ...
    if (pCtx) {
        ...
        chartsCache['otPie'] = new Chart(pCtx, { type: 'doughnut', data: { labels: Object.keys(tOT), datasets: [{ data: Object.values(tOT), backgroundColor: ['#a855f7', '#ec4899', '#6366f1', '#14b8a6', '#f59e0b', '#8b5cf6'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '75%', plugins: { legend: { position: 'bottom', labels: { usePointStyle: true, boxWidth: 8, font: { family: 'Prompt', size: 10 } } } } } });
    }
    if (tCtx) {
        if (chartsCache['otTrend']) chartsCache['otTrend'].destroy();
        chartsCache['otTrend'] = new Chart(tCtx, { type: 'bar', data: { labels: ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'], datasets: [{ label: 'Total OT (Hrs)', data: mOT, backgroundColor: '#d8b4fe', hoverBackgroundColor: '#a855f7', borderRadius: 6, barPercentage: 0.6 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, grid: { borderDash: [4, 4], color: '#f1f5f9' }, ticks: { font: { family: 'Prompt', size: 10 } } }, x: { grid: { display: false }, ticks: { font: { family: 'Prompt', size: 10 } } } }, plugins: { legend: { display: false }, tooltip: { backgroundColor: '#1e293b', padding: 10, titleFont: { family: 'Prompt' }, bodyFont: { family: 'Prompt' }, cornerRadius: 8 } } } });
    }
}
...
function renderHomeActivityTable() {
    ...
    $html('home-activity-tbody', logs.length === 0 ? '<tr><td colspan="6" class="text-center py-4 text-gray-300 italic text-xs">No activity found today</td></tr>' : logs.slice(0, 10).map(l => {
        const teamC = l.team==='MED'?'bg-blue-50 text-blue-600':l.team==='LAB'?'bg-cyan-50 text-cyan-600':l.team==='TES'?'bg-violet-50 text-violet-600':'bg-emerald-50 text-emerald-600';
        ...
        return `<tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors"><td class="p-4 font-mono text-xs text-gray-500">${l.time || l.ts || '-'}</td><td class="p-4"><span class="px-2 py-0.5 rounded bg-gray-100 text-[10px] font-bold ${teamC}">${l.team||'-'}</span></td><td class="p-4 font-bold text-xs text-gray-700 flex items-center gap-2"><div class="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] text-gray-500"><i class="fas fa-user"></i></div>${l.name||l.user||l.staff||l.username||'Unknown'}</td><td class="p-4">${l.type==='IN'?'<span class="bg-emerald-100 text-emerald-600 px-2 py-0.5 rounded text-[9px] font-bold"><i class="fas fa-sign-in-alt mr-1"></i>IN</span>':'<span class="bg-rose-100 text-rose-600 px-2 py-0.5 rounded text-[9px] font-bold"><i class="fas fa-sign-out-alt mr-1"></i>OUT</span>'}</td><td class="p-4 text-xs text-gray-500 truncate max-w-[150px]">${l.job||'-'}</td><td class="p-4 text-right pr-6 flex justify-end items-center">${actionBtn}</td></tr>`;
    }).join(''));
    ...
    if(cCtx) {
        ...
        chartsCache['checkinPie'] = new Chart(cCtx, { type: 'doughnut', data: { labels:['MED','LAB','EHS','TES'], datasets:[{data:[cS.MED, cS.LAB, cS.EHS, cS.TES], backgroundColor:['#004aad','#19a7ce','#0fc1a1','#7c3aed'], borderWidth:0}] }, options: { responsive:true, maintainAspectRatio:false, cutout:'70%', plugins:{ legend:{display:false} } } });
        
        let total = cS.MED + cS.LAB + cS.EHS + cS.TES;
        $html('m-checkin-stats', ['MED','LAB','EHS','TES'].map(t => {
            let color = t==='LAB'?'cyan':t==='EHS'?'emerald':t==='TES'?'violet':'blue';
...
function renderHomeWeeklySection(data) {
    ...
    $html('home-weekly-grid', !d?.length ? '<div class="col-span-3 text-center text-gray-300 text-xs py-8 border-2 border-dashed border-gray-100 rounded-xl">No weekly reports found.</div>' : d.map(r => {
        ...
        const tCls = r.team==='MED'?'text-blue-600 bg-blue-50':r.team==='LAB'?'text-cyan-600 bg-cyan-50':r.team==='EHS'?'text-emerald-600 bg-emerald-50':r.team==='TES'?'text-violet-600 bg-violet-50':'text-gray-600 bg-gray-50';
        return `<div class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col cursor-pointer group" onclick='if(typeof viewDetail==="function") viewDetail(${JSON.stringify(r)})'>${photoHtml}<div class="p-4 flex-1 flex flex-col"><div class="flex justify-between items-start mb-2"><span class="text-[9px] font-bold ${tCls} px-2 py-0.5 rounded uppercase">${r.team}</span><span class="text-[9px] font-bold ${r.status==='Finish'?'bg-green-100 text-green-700':'bg-yellow-50 text-yellow-700'} px-2 py-0.5 rounded-full">${r.status}</span></div><h4 class="font-bold text-gray-800 text-xs mb-2 leading-snug line-clamp-2 group-hover:text-indigo-600 transition-colors">${r.job}</h4><div class="mt-auto pt-2 border-t border-dashed border-gray-100"><div class="mb-2"><div class="flex justify-between text-[9px] mb-1"><span class="text-gray-400 font-bold"><i class="fas fa-microchip mr-1"></i> Device</span><span class="font-bold text-gray-600">${r.deviceAct}/${r.deviceTotal}</span></div><div class="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden"><div class="bg-blue-400 h-1.5 rounded-full transition-all duration-500" style="width: ${Math.min(100, Math.round((r.deviceAct/r.deviceTotal)*100)||0)}%"></div></div></div><div class="mb-3"><div class="flex justify-between items-center mb-1"><span class="text-[9px] text-gray-400 font-bold"><i class="fas fa-chart-line mr-1"></i> Job</span><span class="text-[9px] font-bold text-gray-600">${r.progressText}</span></div><div class="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden"><div class="h-1.5 rounded-full bg-indigo-400" style="width: ${r.progressVal}%"></div></div></div><div class="flex items-center gap-2"><div class="w-5 h-5 rounded-full bg-gray-100 flex items-center justify-center text-[8px] font-bold text-gray-500">${r.reporter.charAt(0)}</div><span class="text-[10px] text-gray-400 truncate max-w-[80px]">${r.reporter.split(' ')[0]}</span></div></div></div></div>`;
    }).join(''));
}
