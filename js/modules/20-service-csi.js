...
function applyServiceFilters() {
    ...
    ['All','MED','LAB','EHS','TES'].forEach(t => {
        ...
        if(btn) {
            if(sFilters.team === t) {
                btn.className = "px-4 py-2 rounded-lg text-xs font-bold text-white bg-indigo-600 shadow-md transform scale-105 transition-all";
            } else {
...
function renderCustomerList() {
    ...
    filteredCustomers.forEach(c => {
        const teamShort = S_TEAM_MAP[c.team] || c.team;
        const teamColor = COLORS[teamShort] || '#94a3b8';
...
function renderSummaryPage() {
    ...
    if(tbody) {
        ...
        configRows.forEach(r => {
            let ySum=0, yCnt=0;
            const rowBgColor = r.type === 'Total' ? 'bg-gray-50/50' : (r.type === 'Com' ? 'bg-blue-50/30' : 'bg-purple-50/30');
            ...
            const pctColorClass = r.type === 'Com' ? 'text-blue-400' : (r.type === 'Net' ? 'text-purple-400' : 'text-gray-400');
    ...
    dataToRender.forEach(d => {
        try {
            ...
            if(comms.length && boxes[team]) {
                const styleClass = team === 'MED' ? 'border-l-blue-600' : (team === 'LAB' ? 'border-l-cyan-500' : (team === 'EHS' ? 'border-l-emerald-500' : 'border-l-purple-500'));
                comms.forEach(c => {
                    const html = `
...
function updateAnalysisSection() {
    ...
    for(let i = 0; i < 5; i++) {
        ...
        if(p !== 0 && c !== 0) {
            ...
            if(diff > 0) colorClass = "text-emerald-600 bg-emerald-50";
            else if(diff < 0) colorClass = "text-rose-600 bg-rose-50";
        }
        ...
        data: {
            ...
            datasets: [
                ...
                },
                {
                    ...
                    label: lbl2, 
                    data: scores2,
                    backgroundColor: ['#1e3a8a', '#1d4ed8', '#3b82f6', '#60a5fa', '#93c5fd'],
                    borderRadius: 4,
                    barPercentage: 0.6,
                    categoryPercentage: 0.8,
                    order: 2
                    ...
                    labels: { 
                        ...
                        generateLabels: function(chart) {
                            return [
                                ...
                                { text: lbl2, fillStyle: '#3b82f6', strokeStyle: '#3b82f6' },
                                { text: 'Trend', fillStyle: '#f59e0b', strokeStyle: '#f59e0b'}
                            ];
                        }
                    }
                },
