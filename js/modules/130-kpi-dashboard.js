...
function renderKPITable() {
    ...
    tbody.innerHTML = filtered.map(row => {
        ...
        let timelineCell = '';
        if (row.isFinished) {
            timelineCell = `<div class="bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-1.5 rounded-xl text-[11px] font-black text-center shadow-sm flex flex-col justify-center w-16 mx-auto"><i class="fas fa-check mb-0.5"></i>DONE</div>`;
        } else if (row.daysLate > 0) {
            timelineCell = `<div class="bg-rose-100 text-rose-700 border border-rose-200 px-2 py-1.5 rounded-xl text-xs font-black text-center shadow-sm flex flex-col justify-center w-16 mx-auto"><span class="text-[8px] uppercase opacity-70 mb-0.5">LATE</span>+${row.daysLate}d</div>`;
        } else {
            timelineCell = `<div class="bg-amber-50 text-amber-600 border border-amber-200 px-2 py-1.5 rounded-xl text-xs font-black text-center shadow-sm flex flex-col justify-center w-16 mx-auto"><span class="text-[8px] uppercase opacity-70 mb-0.5">LEFT</span>${Math.abs(row.daysLate)}d</div>`;
        }
        ...
        const wtColor = isNet
            ...
            : (isCom ? 'bg-rose-50 text-rose-600 border border-rose-100' : 'bg-slate-50 text-slate-600 border border-slate-200');
...
function kpiStatusUi(row) {
    ...
    let statusBg = isDone ? 'bg-emerald-50 border-emerald-200 text-emerald-700'
        : isEdit ? 'bg-orange-50 border-orange-200 text-orange-700'
        : isLate ? 'bg-rose-50 border-rose-200 text-rose-700'
        : 'bg-slate-50 border-slate-200 text-slate-700';

    let statusIcon = isDone ? 'fa-check-circle text-emerald-500'
        : isEdit ? 'fa-screwdriver-wrench text-orange-500'
        : isLate ? 'fa-exclamation-circle text-rose-500'
            ...
            ${steps.map((s, i) => `
                <div class="flex flex-col items-center flex-1 relative">
                    ...
                    <div class="z-10 w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold transition-all shadow-sm
                        ${s.edit ? 'bg-amber-400 text-white ring-4 ring-amber-100' : s.d ? 'bg-emerald-500 text-white shadow-md' : 'bg-slate-100 text-slate-300 border-2 border-slate-200'}">
                    ...
                    ${s.date && s.d && !s.edit ? `<span class="text-[8px] font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded mt-1 border border-emerald-100 shadow-sm">${kpiEsc(s.date)}</span>` : ''}
                </div>
                ${i < steps.length - 1 ? `<div class="flex-1 h-[3px] mt-4 ${s.d ? 'bg-emerald-400' : 'bg-slate-200'}"></div>` : ''}
            `).join('')}
        </div>`;
...
function renderLateJobsUI() {
    ...
    target.innerHTML = lateJobs.length === 0
        ...
        : lateJobs.map(job => `<div onclick='closeModal("modal-kpi-late"); openUpdateModal(${JSON.stringify(job).replace(/'/g, "\\'")})' class="bg-white p-4 rounded-xl shadow-sm border border-rose-100 mb-3 flex justify-between items-center cursor-pointer hover:border-rose-300 transition-all">
            <div>
            ...
            <div class="text-right"><span class="text-[11px] font-black text-rose-600 bg-rose-50 px-3 py-1.5 rounded border border-rose-100 shadow-sm">LATE +${job.daysLate} Days</span></div>
        </div>`).join('');
}
...
function kpiStatusMiniClass(status) {
    if (status === 'ส่ง Report เสร็จแล้ว') return { box: 'bg-emerald-50 border-emerald-100', text: 'text-emerald-700' };
    if (status === 'รอแก้ไข') return { box: 'bg-orange-50 border-orange-100', text: 'text-orange-700' };
    if (status === 'รอส่ง Report') return { box: 'bg-cyan-50 border-cyan-100', text: 'text-cyan-700' };
    if (status === 'กำลังตรวจ' || status === 'ตรวจเสร็จ') return { box: 'bg-indigo-50 border-indigo-100', text: 'text-indigo-700' };
...
function renderKpiExecutiveSummary() {
    ...
    const cards = [
        { label: 'Active Jobs', value: active, note: 'ยังไม่ส่ง Report เสร็จ', icon: 'fa-briefcase', color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-100' },
        { label: 'Action Required / Risk', value: actionRisk, note: 'Overdue / Due today / รอแก้ไข', icon: 'fa-triangle-exclamation', color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-100' },
        { label: 'All Work ทั้งหมด', value: rows.length, note: 'จำนวนงานทั้งหมดในระบบ', icon: 'fa-layer-group', color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-100' },
        { label: 'รอแก้ไข', value: waitingEdit, note: 'รายการที่ถูกตีกลับให้แก้ไข', icon: 'fa-screwdriver-wrench', color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-100' }
    ];
    ...
    const stageCards = [
        ...
        { key: 'pendingSup', label: 'Pending SUP', sub: 'เสร็จพร้อมตรวจ / กำลังตรวจ', value: stageCounts.pendingSup || 0, icon: 'fa-user-check', cls: 'bg-indigo-50 border-indigo-100 text-indigo-700' },
...
function getKpiProgressInfo(row) {
    const statusBg = row.isFinished ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : (row.daysLate > 0 ? 'bg-rose-50 border-rose-200 text-rose-700' : 'bg-slate-50 border-slate-200 text-slate-700');
    const statusIcon = row.isFinished ? 'fa-check-circle text-emerald-500' : (row.daysLate > 0 ? 'fa-exclamation-circle text-rose-500' : 'fa-spinner fa-spin text-amber-500');
...
function getKpiTimelineHtml(row) {
    if (row.isFinished) return `<div class="bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-1.5 rounded-xl text-[11px] font-black text-center shadow-sm flex flex-col justify-center w-16 mx-auto"><i class="fas fa-check mb-0.5"></i>DONE</div>`;
    if (row.daysLate > 0) return `<div class="bg-rose-100 text-rose-700 border border-rose-200 px-2 py-1.5 rounded-xl text-xs font-black text-center shadow-sm flex flex-col justify-center w-16 mx-auto"><span class="text-[8px] uppercase opacity-70 mb-0.5">LATE</span>+${row.daysLate}d</div>`;
    return `<div class="bg-amber-50 text-amber-600 border border-amber-200 px-2 py-1.5 rounded-xl text-xs font-black text-center shadow-sm flex flex-col justify-center w-16 mx-auto"><span class="text-[8px] uppercase opacity-70 mb-0.5">LEFT</span>${Math.abs(row.daysLate)}d</div>`;
}

function getKpiWorkTypeClass(v) {
    const t = String(v || '').toLowerCase();
    if (t.includes('network')) return 'bg-emerald-50 text-emerald-600 border border-emerald-100';
    if (t.includes('commercial')) return 'bg-rose-50 text-rose-600 border border-rose-100';
...
function renderKpiExecutiveSummary() {
    ...
    const cards = [
        ...
        { label: 'Action Required / Risk', value: actionRisk, note: 'Overdue / Due today / รอแก้ไข', icon: 'fa-triangle-exclamation', color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-100', action: '' },
        { label: 'All Work ทั้งหมด', value: rows.length, note: scopeNote + ' • จำนวนงานทั้งหมด', icon: 'fa-layer-group', color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-100', action: '' },
        { label: 'รอแก้ไข', value: waitingEdit, note: 'รายการที่ถูกตีกลับให้แก้ไข', icon: 'fa-screwdriver-wrench', color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-100', action: "kpiQuickStatusFilter('รอแก้ไข')" }
    ];
...
function renderKPITable() {
    ...
    tbody.innerHTML = filtered.map(row => {
        ...
        const serviceCls = serviceBadge === 'ENV' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-indigo-50 text-indigo-600 border-indigo-100';
...
function kpiGetPerformanceResult(row) {
    ...
    if (row?.isFinished && deadline && finishDate) {
        ...
        if (diffDays > 0) {
            return { key:'late', label:'เกิน KPI', short:'เกิน', days:diffDays, finishDate:finishDateRaw, icon:'fa-triangle-exclamation', box:'bg-rose-50 text-rose-700 border-rose-200' };
        }
        if (diffDays === 0) {
            return { key:'on', label:'ตรง KPI', short:'ตรง', days:0, finishDate:finishDateRaw, icon:'fa-bullseye', box:'bg-amber-50 text-amber-700 border-amber-200' };
        }
        return { key:'early', label:'ก่อน KPI', short:'ก่อน', days:Math.abs(diffDays), finishDate:finishDateRaw, icon:'fa-check-circle', box:'bg-emerald-50 text-emerald-700 border-emerald-200' };
    }

    const daysLate = Number(row?.daysLate || 0);
    if (!row?.isFinished && daysLate > 0) {
        return { key:'late', label:'เกิน KPI', short:'เกิน', days:daysLate, finishDate:'', icon:'fa-clock', box:'bg-rose-50 text-rose-700 border-rose-200' };
    }
    if (!row?.isFinished && daysLate === 0) {
        return { key:'on', label:'ตรง KPI', short:'ตรง', days:0, finishDate:'', icon:'fa-bullseye', box:'bg-amber-50 text-amber-700 border-amber-200' };
    }
    return { key:'early', label:'ก่อน KPI', short:'ก่อน', days:Math.abs(daysLate), finishDate:'', icon:'fa-hourglass-half', box:'bg-emerald-50 text-emerald-700 border-emerald-200' };
}
