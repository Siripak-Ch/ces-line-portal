...
function renderJobList() {
    ...
    container.innerHTML = jobs.map(job => {
        const teamColor = job.team === 'MED' ? 'bg-[#0046a3]' : (job.team === 'LAB' ? 'bg-[#006cff]' : 'bg-[#0fc1a1]');
        const count = job.totalPeople || 0;
        
        let statusText = job.status || 'Wait';
        let statusBadge = '';
        if (statusText === 'Finished') {
            statusBadge = `<span class="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-md text-[9px] font-bold border border-emerald-200 uppercase"><i class="fas fa-check-circle"></i> Finished</span>`;
        ...
        let outBtn = '';
        if (hasIN) {
            outBtn = `<button onclick="openActionModal('${job.uniqueKey}', 'OUT')" class="py-2 bg-rose-500 hover:bg-rose-600 text-white rounded-lg text-[9px] font-bold shadow transition-transform active:scale-95 flex items-center justify-center gap-1"><i class="fas fa-sign-out-alt"></i> OUT</button>`;
        } else {
        ...
        return `<div class="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all group flex flex-col h-full">
            ...
            <p class="text-[9px] text-gray-400 flex items-center gap-1 mb-2 truncate"><i class="fas fa-map-marker-alt text-pink-400"></i> ${job.location || 'Unknown'}</p>
            ...
            <div class="grid grid-cols-2 gap-1.5 pt-2 border-t border-gray-50">
                <button onclick="openActionModal('${job.uniqueKey}', 'IN')" class="py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg text-[9px] font-bold shadow transition-transform active:scale-95 flex items-center justify-center gap-1"><i class="fas fa-sign-in-alt"></i> IN</button>
                ${outBtn}
            </div>
        </div>`;
    }).join('');
...
function showDetailModal(people) {
    ...
    if (!people || people.length === 0) { list.innerHTML = `<div class="flex flex-col items-center py-8 text-gray-300"><i class="fas fa-user-slash text-3xl mb-2"></i><span class="text-xs">No activity yet.</span></div>`;
        ...
        list.innerHTML = people.map(p => {
            ...
            const icon = isWorking ? 'fa-clock text-blue-500' : 'fa-check-circle text-green-500'; 
            const statusText = isWorking ? 'On-Site' : 'Finished';
            return `<div class="flex items-center justify-between p-3 bg-white border border-gray-100 rounded-xl mb-2 shadow-sm"><div class="flex items-center gap-3"><div class="w-9 h-9 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center font-bold text-xs text-indigo-600">${p.name.charAt(0)}</div><div><p class="text-xs font-bold text-gray-800">${p.name}</p><p class="text-[9px] text-gray-400 font-bold uppercase flex items-center gap-1"><i class="fas ${icon}"></i> 
            ${statusText}</p></div></div><div class="text-right flex flex-col gap-1"><span class="text-[9px] font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-md border border-emerald-200">IN: ${p.inTime}</span><span class="text-[9px] font-bold text-rose-700 bg-rose-100 px-2 py-0.5 rounded-md border border-rose-200">OUT: ${p.outTime}</span></div></div>`;
        }).join('');
    }
...
function filterActivityTable() {
    ...
    container.innerHTML = logs.map(row => {
        ...
        return `<tr onclick='openLogDetail(${JSON.stringify(row).replace(/'/g, "\\'")})' class="hover:bg-gray-50 transition-colors border-b last:border-0 border-gray-100 cursor-pointer"><td class="p-4 font-bold text-gray-600 text-xs">${time} <span class="ml-1 text-[9px] text-gray-400">${row.date}</span></td><td class="p-4 font-bold text-xs"><span class="${row.team === 'MED' ? 'text-blue-600' : row.team === 'LAB' ? 'text-cyan-600' : 'text-teal-600'}">${row.team}</span></td><td class="p-4 font-bold text-gray-800 text-xs">${row.user}</td><td class="p-4"><span class="px-2 py-1 rounded text-[9px] font-bold ${badge}">${row.type}</span></td><td class="p-4 text-gray-500 text-xs truncate max-w-[150px]" title="${row.job}">${row.job}</td><td class="p-4 text-center">${photoHtml}</td><td class="p-4">${gpsHtml}</td></tr>`;
    }).join('');
}
...
function openActionModal(key, type) {
    ...
    if(type === 'IN') { 
        document.getElementById('modal-header-bg').className = 'bg-gradient-to-r from-emerald-500 to-teal-600 p-6 text-center relative'; document.getElementById('modal-title').innerText = 'Check In';
        btn.className = 'w-full py-3.5 rounded-2xl font-bold text-white shadow-xl bg-emerald-500 hover:bg-emerald-600 transition-all text-sm'; btn.innerText = 'Confirm IN';
    } else { 
        document.getElementById('modal-header-bg').className = 'bg-gradient-to-r from-rose-500 to-pink-600 p-6 text-center relative'; document.getElementById('modal-title').innerText = 'Check Out';
        btn.className = 'w-full py-3.5 rounded-2xl font-bold text-white shadow-xl bg-rose-500 hover:bg-rose-600 transition-all text-sm'; btn.innerText = 'Confirm OUT';
    }
...
function attemptGetPosition(txt, ind, btn) {
    gpsAttempts++;
    txt.innerHTML = `<i class="fas fa-satellite-dish fa-spin text-orange-500"></i> Locating... (Try ${gpsAttempts}/${MAX_GPS_ATTEMPTS})`;
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
            ...
            if (err.code === 1) { 
                ...
                ind.innerHTML = '<i class="fas fa-ban text-rose-500"></i>';
                btn.disabled = false;
                ...
                ind.innerHTML = '<i class="fas fa-exclamation-triangle text-rose-500"></i>';
                btn.disabled = false;
                document.getElementById('manual-loc-container').classList.remove('hidden');
            }
        },
...
function processValidPosition(p, txt, ind, btn) {
    ...
    txt.innerHTML = `<i class="fas fa-circle-notch fa-spin text-purple-500"></i> Found (+/-${acc}m). Getting Address...`;
    
    google.script.run.withSuccessHandler(res => {
        if(res.success) {
            currentLocationName = res.address;
            const accText = acc <= 100 ? `<span class="text-green-600">High (${acc}m)</span>` : `<span class="text-orange-500">Avg (${acc}m)</span>`;
            ...
            ind.innerHTML = '<i class="fas fa-map-marker-alt text-emerald-500"></i>';
            ind.className = 'w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 shrink-0 shadow-sm border border-emerald-200';
            btn.disabled = false;
        } else {
    ...
    google.script.run.withSuccessHandler(res => {
        ...
        if(res.success) {
            ...
            if(sum) {
                ...
                if(isIN) {
                    header.className = 'h-20 w-full bg-gradient-to-r from-emerald-500 to-teal-600 flex items-center justify-center';
                    ...
                    header.className = 'h-20 w-full bg-gradient-to-r from-rose-500 to-orange-600 flex items-center justify-center';
...
function resetCheckinDB() { Swal.fire({ title: 'Confirm Reset?', text: "Clear 'Checkinout' sheet?", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Yes, Reset' }).then((result) => { if (result.isConfirmed) { google.script.run.withSuccessHandler(() => { Swal.fire('Reset Complete', '', 'success'); loadCheckinData(); }).clearCheckinData(); } }); }
...
function openLogDetail(data) {
    ...
    iconDiv.className = `w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold text-white shadow-md ${isIN ? 'bg-emerald-500' : 'bg-rose-500'}`;
