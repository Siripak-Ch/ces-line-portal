...
function initStockInventoryModule(force=false){
  ...
  google.script.run.withSuccessHandler(res=>{if(!res||!res.success){Swal.fire('Inventory Error',(res&&res.message)||'Cannot load inventory','error');return;}SI.loaded=true;SI.raw=res;SI.inv=res.inventory||[];SI.acc=res.accessories||[];si_fillFilters();si_renderKpi();si_applyFilters();}).withFailureHandler(err=>Swal.fire('Inventory Error',err.message||String(err),'error')).si_getStockInventoryData(force===true);
}
function si_fillFilters(){const f=SI.raw.filters||{};si_fillSelect('siBrand',f.brands,'All Brand');si_fillSelect('siModel',f.models,'All Model');si_fillSelect('siLocation',f.locations,'All Location');si_fillSelect('siStatus',f.statuses,'All Status');si_fillSelect('siAccType',f.accessoryTypes,'All Type');}
...
function si_renderKpi(){const k=SI.raw.kpi||{};const items=[['อุปกรณ์หลักทั้งหมด',k.total,'fa-microchip','#2563eb','#dbeafe'],['STOCK',k.stock,'fa-warehouse','#059669','#d1fae5'],['IN-USE',k.inUse,'fa-arrow-right-from-bracket','#d97706','#fef3c7'],['OVERDUE/MISSING/BROKEN',k.risk,'fa-triangle-exclamation','#dc2626','#fee2e2'],['Accessories',k.accessories,'fa-plug','#0ea5e9','#e0f2fe'],['Low Stock Acc.',k.accLow,'fa-battery-quarter','#ea580c','#ffedd5']];spSetHtml('siKpiGrid',items.map(i=>`<div class="sp-kpi"><div class="ico" style="background:${i[4]}"><i class="fas ${i[2]}" style="color:${i[3]}"></i></div><div class="label">${i[0]}</div><div class="val" style="color:${i[3]}">${spNum(i[1])}</div></div>`).join(''));}
function si_applyFilters(){if(SI.tab==='equip'){const q=spVal('siSearch','').toLowerCase(),b=spVal('siBrand','all'),m=spVal('siModel','all'),l=spVal('siLocation','all'),s=spVal('siStatus','all');SI.filtered=SI.inv.filter(d=>{const text=[d.idCode,d.sn,d.brand,d.model,d.itemName,d.location,d.status,d.borrower,d.actionRequired].join(' ').toLowerCase();if(q&&!text.includes(q))return false;if(b!=='all'&&d.brand!==b)return false;if(m!=='all'&&d.model!==m)return false;if(l!=='all'&&d.location!==l)return false;if(s!=='all'&&d.status!==s)return false;return true;});SI.page=1;si_renderTable();}else{const q=spVal('siAccSearch','').toLowerCase(),t=spVal('siAccType','all'),st=spVal('siAccStatus','all');SI.accFiltered=SI.acc.filter(a=>{const text=[a.idCode,a.name,a.type,a.status,a.location,a.remark].join(' ').toLowerCase();if(q&&!text.includes(q))return false;if(t!=='all'&&a.type!==t)return false;if(st!=='all'&&a.status!==st)return false;return true;});SI.accPage=1;si_renderAccCards();}}
...
function si_deletePrompt(d){Swal.fire({title:'Delete / Hide '+d.idCode+'?',text:'รายการจะถูกซ่อนจาก inventory โดยตั้งสถานะ DELETED',icon:'warning',showCancelButton:true,confirmButtonText:'Delete/Hide',confirmButtonColor:'#dc2626'}).then(r=>{if(!r.isConfirmed)return;google.script.run.withSuccessHandler(si_afterAction).withFailureHandler(si_actionError).si_deleteEquipment({idCode:d.idCode,reason:'Deleted/Hidden from web'});});}
...
function si_fillFilters(){
  ...
  si_fillSelect('siStatus',f.statuses,'All Status');
...
function si_renderKpi(){
  siEnsureStyleV8();
  const k=SI.raw.kpi||{};
  const eqItems=[['อุปกรณ์หลักทั้งหมด',k.total,'fa-microchip','#2563eb','#dbeafe'],['STOCK',k.stock,'fa-warehouse','#059669','#dcfce7'],['IN-USE',k.inUse,'fa-arrow-right-from-bracket','#d97706','#fef3c7'],['OVERDUE',k.overdue||0,'fa-clock','#dc2626','#fee2e2'],['MISSING',k.missing||0,'fa-circle-question','#854d0e','#fef9c3'],['BROKEN',k.broken||0,'fa-screwdriver-wrench','#64748b','#e2e8f0'],['RECHECK',k.recheck||0,'fa-triangle-exclamation','#7c3aed','#ede9fe'],['RISK',k.risk,'fa-triangle-exclamation','#dc2626','#fee2e2']];
  const accItems=[['Accessories',k.accessories,'fa-plug','#0ea5e9','#e0f2fe'],['Total Stock',k.accTotalStock||0,'fa-boxes-stacked','#059669','#dcfce7'],['Low Stock',k.accLow,'fa-battery-quarter','#ea580c','#ffedd5'],['Pending Approval',k.accPending||0,'fa-bell','#7c3aed','#ede9fe'],['Teams',k.accTeams||0,'fa-users','#2563eb','#dbeafe']];
...
function si_accCard(a,i){const id=a.accessoryId||a.idCode||a.itemName||a.name;const qty=Number(a.stockQty||a.qty||0),min=Number(a.minStockQty||a.minStock||0);const low=qty<=min;return `<div class="sp-acc-card"><div class="sp-acc-head"><div class="sp-acc-icon ${si_teamClass(a.team)}"><i class="fas ${si_accIcon(a)}"></i></div><div><div class="sp-acc-title">${spEsc(a.itemName||a.name||'-')}</div><div class="sp-acc-meta">${spEsc(a.team||'-')} • ${spEsc(id)}</div></div></div><div><div class="sp-acc-stock" style="color:${low?'#dc2626':'#059669'}">${spNum(qty)}</div><div class="sp-muted">คงเหลือ / Min ${spNum(min)}</div></div><div>${low?'<span class="sp-badge Overdue">LOW</span>':spBadge(a.status||'STOCK')} <span class="sp-pill">${spEsc(a.actionRequired||a.action_required||'No action')}</span></div><div class="sp-acc-actions"><input id="siAccQty_${i}" type="number" min="1" max="${qty}" value="1"><button class="sp-btn success" onclick='si_addAccessoryToCart(${JSON.stringify(a).replace(/'/g,"&#39;")},"siAccQty_${i}")'><i class="fas fa-cart-plus"></i> ใส่ตะกร้า</button><button class="sp-btn warn" onclick='si_restockPrompt(${JSON.stringify(a).replace(/'/g,"&#39;")})'><i class="fas fa-box-open"></i></button></div></div>`;}
...
function si_fillFilters(){
  ...
  si_fillSelect('siBrand',f.brands,'All Brand');si_fillSelect('siModel',f.models,'All Model');si_fillSelect('siLocation',f.locations,'All Location');si_fillSelect('siStatus',f.statuses,'All Status');
...
function si_renderKpi(){
  ...
  const equip=[['อุปกรณ์หลักทั้งหมด',k.total,'fa-microchip','#2563eb','#dbeafe'],['STOCK',k.stock,'fa-warehouse','#059669','#d1fae5'],['IN-USE',k.inUse,'fa-arrow-right-from-bracket','#d97706','#fef3c7'],['OVERDUE/MISSING/BROKEN',k.risk,'fa-triangle-exclamation','#dc2626','#fee2e2']];
  ...
  const acc=[['ACCESSORIES',accTotal,'fa-plug','#0ea5e9','#e0f2fe'],['TOTAL STOCK',accStock,'fa-boxes-stacked','#059669','#d1fae5'],['LOW STOCK',low,'fa-battery-quarter','#ea580c','#ffedd5'],['PENDING APPROVAL',pending,'fa-bell','#7c3aed','#ede9fe']];
...
function si_accCard(a,i){
  ...
  return `<div class="sp-acc-card-v11 ${low?'is-low':''}"><div class="sp-acc-card-head-v11"><div class="sp-acc-icon-v11 ${si_teamClass(a.team)}"><i class="fas ${si_accIcon(a)}"></i></div><div><div class="sp-acc-name-v11">${spEsc(a.itemName||a.name||'-')}</div><div class="sp-acc-meta-v11">${spEsc(a.team||'-')} • ${spEsc(id||'-')}</div></div></div><div class="sp-acc-stock-v11"><div><div class="num" style="color:${low?'#dc2626':'#059669'}">${spNum(qty)}</div><div class="sp-muted">คงเหลือ / Min ${spNum(min)}</div></div><div>${low?'<span class="sp-chip low">LOW STOCK</span>':spBadge(a.status||'STOCK')}</div></div><div class="sp-muted" style="min-height:18px">${spEsc(a.actionRequired||a.action_required||'No action')}</div><div class="sp-acc-control-v11"><input id="siAccQty_${i}" type="number" min="1" max="${Math.max(1,qty)}" value="1"><button class="sp-btn success" ${low?'disabled style="opacity:.5;cursor:not-allowed"':''} onclick='si_addAccessoryToCart(${JSON.stringify(a).replace(/'/g,"&#39;")},"siAccQty_${i}")'><i class="fas fa-cart-plus"></i> ใส่ตะกร้า</button><button class="sp-btn warn" title="Restock" onclick='si_restockPrompt(${JSON.stringify(a).replace(/'/g,"&#39;")})'><i class="fas fa-box-open"></i></button></div>${low?'<div class="sp-muted" style="color:#c2410c;margin-top:8px;font-weight:900">ต้อง Restock ก่อนเบิกใช้</div>':''}</div>`;
}
...
function si_v12Style(){
  ...
  st.textContent=`
    #view-inventory .sp-cart-fab{left:24px!important;right:auto!important;bottom:28px!important;width:60px!important;height:60px!important;z-index:190!important;background:linear-gradient(135deg,#2563eb,#1d4ed8)!important;box-shadow:0 16px 34px rgba(37,99,235,.38)!important}
    #view-inventory .sp-cart-fab.is-open{background:linear-gradient(135deg,#0f172a,#334155)!important}
    ...
    #view-inventory .sp-icon-btn{width:27px!important;height:27px!important;border-radius:8px!important;font-size:11px!important;padding:0!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;border:1px solid #bfdbfe;background:#eff6ff;color:#2563eb;cursor:pointer;transition:.16s}
    #view-inventory .sp-icon-btn:hover{transform:translateY(-1px);filter:brightness(.98)}
    #view-inventory .sp-icon-btn.orange{background:#fff7ed!important;border-color:#fed7aa!important;color:#ea580c!important}
    #view-inventory .sp-icon-btn.green{background:#ecfdf5!important;border-color:#a7f3d0!important;color:#059669!important}
    #view-inventory .sp-icon-btn.gray{background:#f8fafc!important;border-color:#e2e8f0!important;color:#475569!important}
    #view-inventory .sp-icon-btn.red{background:#fff1f2!important;border-color:#fecdd3!important;color:#e11d48!important}
    #view-inventory .sp-acc-grid-v12{display:grid;grid-template-columns:repeat(auto-fill,minmax(270px,1fr));gap:14px}
    #view-inventory .sp-acc-card-v12{background:#fff;border:1px solid #e8eef6;border-radius:18px;padding:16px;box-shadow:0 4px 16px rgba(30,58,138,.06);transition:.18s;position:relative;overflow:hidden}
    #view-inventory .sp-acc-card-v12:hover{transform:translateY(-2px);box-shadow:0 10px 24px rgba(30,58,138,.1)}
    #view-inventory .sp-acc-card-v12.is-low{background:#fff7ed;border-color:#fdba74}
    ...
    #view-inventory .team-med{background:#dbeafe;color:#0046a3}.team-lab{background:#e0f2fe;color:#006cff}.team-ehs{background:#ccfbf1;color:#0f766e}.team-other{background:#f1f5f9;color:#64748b}
    ...
    #view-inventory .sp-acc-stepper-v12{display:grid;grid-template-columns:34px 58px 34px 1fr 38px 38px;gap:7px;align-items:center;margin-top:10px}.sp-acc-stepper-v12 input{height:34px;border:1px solid #dbe3ef;border-radius:10px;text-align:center;font-weight:1000;color:#0f172a}.sp-qty-btn{height:34px;border:1px solid #dbe3ef;background:#f8fbff;border-radius:10px;font-weight:1000;cursor:pointer}.sp-qty-btn:hover{background:#eff6ff}.sp-acc-stepper-v12 .sp-btn{height:34px;padding:0 10px;font-size:12px}.sp-acc-stepper-v12 .sp-mini-btn{height:34px;width:38px;border-radius:10px;border:1px solid #fed7aa;background:#fff7ed;color:#ea580c;cursor:pointer}.sp-acc-stepper-v12 .sp-mini-btn.blue{border-color:#bfdbfe;background:#eff6ff;color:#2563eb}
...
function si_accCard(a,i){
  ...
  return `<div class="sp-acc-card-v12 ${low?'is-low':''}"><div class="sp-acc-head-v12"><div class="sp-acc-icon-v12 ${si_teamClass(a.team)}"><i class="fas ${si_accIcon(a)}"></i></div><div><div class="sp-acc-name-v12">${spEsc(a.itemName||a.name||'-')}</div><div class="sp-acc-meta-v12">${spEsc(a.team||'-')} • ${spEsc(id||'-')}</div></div></div><div class="sp-acc-stock-v12"><div><div class="num" style="color:${low?'#dc2626':'#059669'}">${spNum(qty)}</div><div class="sp-muted">คงเหลือ / Min ${spNum(min)}</div></div><div>${low?'<span class="sp-chip low">LOW STOCK</span>':spBadge(a.status||'STOCK')}</div></div><div class="sp-muted" style="min-height:18px">${spEsc(a.actionRequired||a.action_required||'No action')}</div><div class="sp-acc-stepper-v12"><button class="sp-qty-btn" onclick="si_accQtyChange('${qid}',-1,${Math.max(1,qty)})">−</button><input id="${qid}" type="number" min="1" max="${Math.max(1,qty)}" step="1" value="1" oninput="this.value=Math.max(1,Math.floor(Number(this.value)||1))"><button class="sp-qty-btn" onclick="si_accQtyChange('${qid}',1,${Math.max(1,qty)})">+</button><button class="sp-btn success" ${low?'disabled style="opacity:.5;cursor:not-allowed"':''} onclick='si_addAccessoryToCart(${JSON.stringify(a).replace(/'/g,"&#39;")},"${qid}")'><i class="fas fa-cart-plus"></i> ใส่ตะกร้า</button><button class="sp-mini-btn" title="Restock" onclick='si_restockPrompt(${JSON.stringify(a).replace(/'/g,"&#39;")})'><i class="fas fa-box-open"></i></button><button class="sp-mini-btn blue" title="Edit Min Stock" onclick='si_editMinStockPrompt(${JSON.stringify(a).replace(/'/g,"&#39;")})'><i class="fas fa-sliders"></i></button></div>${low?'<div class="sp-muted" style="color:#c2410c;margin-top:8px;font-weight:900">ต้อง Restock ก่อนเบิกใช้</div>':''}</div>`;
}
function si_addAccessoryToCart(a,inputId){
...
function si_v13Style(){
  ...
  st.textContent=`
    #view-inventory .sp-cart-fab{right:24px!important;left:auto!important;bottom:28px!important;width:62px!important;height:62px!important;border-radius:999px!important;z-index:190!important;background:linear-gradient(135deg,#2563eb,#1d4ed8)!important;color:#fff!important;box-shadow:0 18px 36px rgba(37,99,235,.42)!important;display:flex!important;align-items:center!important;justify-content:center!important;}
    #view-inventory .sp-cart-fab i{font-size:20px!important;}
    #view-inventory .sp-cart-fab.is-open{background:linear-gradient(135deg,#0f172a,#334155)!important;}
    #view-inventory #siCartFabBadge{position:absolute;top:-5px;right:-5px;min-width:22px;height:22px;border-radius:999px;background:#ef4444;color:#fff;font-size:11px;font-weight:1000;display:flex;align-items:center;justify-content:center;padding:0 5px;border:2px solid #fff;}
    #view-inventory #siCartDrawer{right:-520px;transition:right .28s cubic-bezier(.4,0,.2,1);}
    ...
    #view-inventory .sp-cart-detail-card{border:1px solid #dbe3ef;background:#f8fbff;border-radius:14px;padding:12px 14px;margin-bottom:10px;}
...
function si_v14Style(){
  ...
  st.textContent=`
    ...
    #view-inventory .si-icon-only #siCartCount{position:absolute;top:-6px;right:-6px;min-width:20px;height:20px;border-radius:999px;background:#ef4444;color:#fff;font-size:10px!important;line-height:20px;text-align:center;border:2px solid #fff;padding:0 5px;}
    ...
    #view-inventory .sp-cart-fab{left:auto!important;right:24px!important;bottom:26px!important;width:60px!important;height:60px!important;border-radius:999px!important;z-index:190!important;background:linear-gradient(135deg,#2563eb,#1d4ed8)!important;color:#fff!important;box-shadow:0 18px 36px rgba(37,99,235,.42)!important;display:flex!important;align-items:center!important;justify-content:center!important;}
    #view-inventory .sp-cart-fab i{font-size:20px!important;}
    #view-inventory .sp-cart-fab.is-open{background:linear-gradient(135deg,#0f172a,#334155)!important;}
    #view-inventory #siCartFabBadge{position:absolute!important;top:-6px!important;right:-6px!important;min-width:22px!important;height:22px!important;border-radius:999px!important;background:#ef4444!important;color:#fff!important;font-size:11px!important;font-weight:1000!important;display:flex!important;align-items:center!important;justify-content:center!important;padding:0 5px!important;border:2px solid #fff!important;}
    ...
    .si-history-scope-pill{display:inline-flex;align-items:center;gap:6px;background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;border-radius:999px;padding:6px 10px;font-size:11px;font-weight:900;margin-bottom:10px;}
...
function si_v15Style(){
  ...
  st.textContent=`
    #view-inventory .sp-cart-fab{position:fixed!important;right:24px!important;left:auto!important;bottom:24px!important;top:auto!important;width:60px!important;height:60px!important;border-radius:999px!important;z-index:210!important;background:#2563eb!important;color:#fff!important;box-shadow:0 18px 36px rgba(37,99,235,.38)!important;display:flex!important;align-items:center!important;justify-content:center!important;border:0!important;}
    #view-inventory #siCartFabBadge{position:absolute!important;top:-6px!important;right:-6px!important;min-width:22px!important;height:22px!important;border-radius:999px!important;background:#ef4444!important;color:#fff!important;font-size:11px!important;font-weight:900!important;display:flex!important;align-items:center!important;justify-content:center!important;border:2px solid #fff!important;}
    ...
    #view-inventory .si-icon-only #siCartCount,#view-inventory #siLowStockHeaderCount{position:absolute;top:-6px;right:-6px;min-width:20px;height:20px;border-radius:999px;background:#ef4444;color:#fff;font-size:10px!important;font-weight:900;display:flex;align-items:center;justify-content:center;border:2px solid #fff;padding:0 5px;}
    ...
    #view-inventory .sp-acc-row.low{border-color:#fdba74;background:#fff7ed;}
    #view-inventory .sp-acc-icon{width:38px;height:38px;border-radius:12px;display:flex;align-items:center;justify-content:center;background:#dbeafe;color:#2563eb;}
    #view-inventory .sp-acc-icon.med{background:#dbeafe;color:#1d4ed8}#view-inventory .sp-acc-icon.lab{background:#ede9fe;color:#6d28d9}#view-inventory .sp-acc-icon.ehs{background:#d1fae5;color:#047857}#view-inventory .sp-acc-icon.tes{background:#fef3c7;color:#d97706}
    #view-inventory .sp-acc-title{font-weight:900;color:#0f172a;font-size:13px}.sp-acc-sub{font-size:11px;color:#64748b;margin-top:2px}.sp-acc-num{font-weight:1000;font-size:18px}.sp-acc-min{font-size:11px;color:#64748b}.sp-acc-actions{display:flex;align-items:center;gap:6px;flex-wrap:nowrap}.sp-acc-actions input{width:54px;text-align:center;border:1px solid #dbe3ef;border-radius:10px;height:32px;font-weight:900}.sp-mini-btn{width:32px;height:32px;border-radius:10px;border:1px solid #dbe3ef;background:#fff;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;font-size:12px}.sp-mini-btn.green{background:#059669;color:#fff;border-color:#059669}.sp-mini-btn.orange{background:#fff7ed;color:#ea580c;border-color:#fed7aa}.sp-mini-btn.blue{background:#eff6ff;color:#2563eb;border-color:#bfdbfe}.sp-mini-btn:disabled{opacity:.45;cursor:not-allowed}
...
function si_v16Style(){
  ...
  st.textContent=`
    #view-inventory .sp-cart-fab{position:fixed!important;right:24px!important;left:auto!important;bottom:24px!important;top:auto!important;width:58px!important;height:58px!important;border-radius:999px!important;background:#2563eb!important;color:white!important;z-index:220!important;box-shadow:0 16px 30px rgba(37,99,235,.35)!important;display:flex!important;align-items:center!important;justify-content:center!important;border:none!important;}
    #view-inventory #siCartFabBadge{position:absolute!important;top:-8px!important;right:-8px!important;background:#ef4444!important;color:white!important;border-radius:999px!important;min-width:20px!important;height:20px!important;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900;}
    ...
    #view-inventory .sp-acc-row.low{background:#fff7ed;border-color:#fdba74;}
    ...
    #view-inventory .team-med{background:#dbeafe;color:#1d4ed8}#view-inventory .team-lab{background:#e0f2fe;color:#0369a1}#view-inventory .team-ehs{background:#ccfbf1;color:#047857}#view-inventory .team-tes{background:#ede9fe;color:#6d28d9}#view-inventory .team-other{background:#f1f5f9;color:#475569}
    #view-inventory .acc-title{font-weight:900;color:#0f172a;line-height:1.15}.acc-sub{font-size:11px;color:#64748b;margin-top:2px}.acc-stock{font-size:20px;font-weight:900}.acc-min{font-size:11px;color:#64748b}.acc-actions{display:flex;gap:6px;align-items:center;justify-content:flex-end}.acc-actions input{width:58px;height:34px;border:1px solid #cbd5e1;border-radius:10px;text-align:center;font-weight:900;background:#f8fafc}.acc-mini{width:34px;height:34px;border-radius:10px;border:1px solid #cbd5e1;background:#f8fafc;font-weight:900}.acc-mini.green{background:#059669;color:white;border-color:#059669}.acc-mini.orange{background:#fff7ed;color:#ea580c;border-color:#fed7aa}.acc-mini.blue{background:#eff6ff;color:#2563eb;border-color:#bfdbfe}.acc-mini:disabled{opacity:.45;cursor:not-allowed;background:#e2e8f0;color:#64748b;border-color:#cbd5e1}
...
function si_v17Style(){
  ...
  st.textContent=`
    ...
    #siCartFab,#view-inventory .sp-cart-fab{position:fixed!important;right:24px!important;left:auto!important;bottom:24px!important;top:auto!important;width:58px!important;height:58px!important;border-radius:999px!important;background:#2563eb!important;color:#fff!important;z-index:9999!important;box-shadow:0 18px 38px rgba(37,99,235,.35)!important;display:flex!important;align-items:center!important;justify-content:center!important;border:0!important}
    #siCartFabBadge{position:absolute!important;top:-7px!important;right:-7px!important;min-width:22px!important;height:22px!important;border-radius:999px!important;background:#ef4444!important;color:#fff!important;font-size:11px!important;font-weight:900!important;border:2px solid #fff!important;display:flex!important;align-items:center!important;justify-content:center!important}
    #view-inventory .si-icon-only{width:38px!important;height:38px!important;min-width:38px!important;padding:0!important;font-size:0!important;border-radius:12px!important;position:relative!important}
    #view-inventory .si-icon-only i{font-size:14px!important}
...
function si_v28HistoryStyle(){
  ...
  st.textContent=`
    ...
    .si-v28-history-card{display:grid;grid-template-columns:12px 1fr;gap:12px;padding:13px 14px;border:1px solid #e5e7eb;border-radius:16px;background:#f8fafc}
    .si-v28-dot{width:10px;height:10px;border-radius:999px;background:#2563eb;margin-top:5px}
    .si-v28-dot.approved{background:#059669}.si-v28-dot.rejected{background:#ef4444}.si-v28-dot.pending{background:#d97706}
    ...
    .si-v28-meta{font-size:11px;color:#94a3b8;margin-top:5px}
    .si-v28-scope{display:inline-flex;gap:7px;align-items:center;padding:7px 12px;border-radius:999px;background:#eff6ff;color:#2563eb;font-weight:900;font-size:12px;margin-bottom:12px}
  `;
  document.head.appendChild(st);
}
...
function si_v30Style(){
  ...
  st.textContent=`
    ...
    .si-v30-alert-tabs button{border:1px solid #dbeafe;background:#fff;color:#2563eb;border-radius:999px;padding:8px 12px;font-size:12px;font-weight:900;cursor:pointer}
    .si-v30-alert-tabs button.active{background:#2563eb;color:#fff;border-color:#2563eb}
    .si-v30-list{display:flex;flex-direction:column;gap:10px;max-height:62vh;overflow:auto;text-align:left;padding-right:3px}
    .si-v30-row{display:grid;grid-template-columns:34px 1fr auto;gap:12px;align-items:center;border:1px solid #e5e7eb;background:#fff;border-radius:16px;padding:12px}
    .si-v30-dot{width:32px;height:32px;border-radius:12px;display:flex;align-items:center;justify-content:center;background:#eff6ff;color:#2563eb}
    .si-v30-dot.pending{background:#fff7ed;color:#d97706}.si-v30-dot.low{background:#fee2e2;color:#dc2626}
    ...
    .si-v30-actions{display:flex;gap:6px;align-items:center;flex-wrap:wrap;justify-content:flex-end}.si-v30-mini{border:0;border-radius:10px;padding:8px 10px;font-size:12px;font-weight:900;cursor:pointer}.si-v30-mini.ok{background:#059669;color:#fff}.si-v30-mini.bad{background:#ef4444;color:#fff}.si-v30-mini.soft{background:#eff6ff;color:#2563eb;border:1px solid #bfdbfe}
...
function si_v30RejectFromAlert(requestId){
  const user=si_v30CurrentUserPayload();
  Swal.fire({title:'Reject request?',html:`<input id="siV30RejectNote" class="swal2-input" placeholder="Reason / note">`,icon:'warning',showCancelButton:true,confirmButtonText:'Reject',confirmButtonColor:'#ef4444'}).then(r=>{
    if(!r.isConfirmed)return;
...
function si_v31RenderAccessoryKpi(rows, pendingOverride){
  ...
  const accItems = [
    ...
    ['Total Stock', k.accTotalStock, 'fa-boxes-stacked', '#059669', '#dcfce7'],
...
(function(){
  ...
  function ensureStyle(){
    ...
    st.textContent=`
      ...
      .si-ci-chip.monthly{background:#fee2e2;color:#991b1b;border-color:#fecaca}.si-ci-chip.quarterly{background:#fef3c7;color:#92400e;border-color:#fde68a}.si-ci-chip.half{background:#e0f2fe;color:#075985;border-color:#bae6fd}
      .si-ci-due{background:#fee2e2!important;color:#991b1b!important;border-color:#fecaca!important}.si-ci-soon{background:#fff7ed!important;color:#9a3412!important;border-color:#fed7aa!important}.si-ci-ok{background:#dcfce7!important;color:#166534!important;border-color:#bbf7d0!important}
      .si-ci-meta{display:flex;gap:6px;flex-wrap:wrap;margin:10px 0}.si-ci-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:10px;max-height:560px;overflow:auto;text-align:left}.si-ci-row{border:1px solid #e8eef6;border-radius:14px;padding:12px;background:#fff}.si-ci-title{font-weight:1000;color:#0f172a}.si-ci-sub{font-size:12px;color:#64748b;margin-top:4px;line-height:1.45}.si-ci-actions{margin-top:10px;display:flex;gap:8px;flex-wrap:wrap}.si-ci-mini{border:0;border-radius:10px;padding:7px 10px;font-size:11px;font-weight:900;cursor:pointer}.si-ci-mini.primary{background:#2563eb;color:white}.si-ci-mini.warn{background:#fff7ed;color:#c2410c}.si-ci-mini.green{background:#dcfce7;color:#166534}.si-ci-mini.gray{background:#f1f5f9;color:#334155}
    `;
    document.head.appendChild(st);
  }
  ...
  window.si_renderKpi=function(){
    ...
    if(grid && !document.getElementById('siCiKpiPatch')){
      grid.insertAdjacentHTML('beforeend', `<div id="siCiKpiPatch" style="display:contents">
        <div class="sp-kpi"><div class="ico" style="background:#fee2e2"><i class="fas fa-calendar-times" style="color:#dc2626"></i></div><div class="label">Check Due</div><div class="val" style="color:#dc2626">${num(k.accCheckDue||0)}</div></div>
        <div class="sp-kpi"><div class="ico" style="background:#fef3c7"><i class="fas fa-clock" style="color:#d97706"></i></div><div class="label">Due Soon</div><div class="val" style="color:#d97706">${num(k.accCheckSoon||0)}</div></div>
        <div class="sp-kpi"><div class="ico" style="background:#e0f2fe"><i class="fas fa-repeat" style="color:#0284c7"></i></div><div class="label">M / Q / H</div><div class="val" style="color:#0284c7;font-size:18px">${num(k.accMonthly||0)} / ${num(k.accQuarterly||0)} / ${num(k.accHalfYearly||0)}</div></div>
      </div>`);
    }
  };
...
(function(){
  ...
  function ensureStyle(){
    ...
    s.textContent=`
      ...
      .si-v31-monthly{background:#eff6ff;color:#1d4ed8;border-color:#bfdbfe}.si-v31-quarterly{background:#fffbeb;color:#b45309;border-color:#fde68a}.si-v31-half{background:#f8fafc;color:#475569;border-color:#e2e8f0}.si-v31-due{background:#fef2f2;color:#dc2626;border-color:#fecaca}.si-v31-soon{background:#fff7ed;color:#ea580c;border-color:#fed7aa}.si-v31-ok{background:#ecfdf5;color:#059669;border-color:#a7f3d0}
      .si-v31-alert-wrap{max-height:560px;overflow:auto;border:1px solid #e5e7eb;border-radius:14px}.si-v31-alert-table{width:100%;border-collapse:separate;border-spacing:0;font-size:12px}.si-v31-alert-table th{position:sticky;top:0;background:#f8fafc;color:#475569;text-align:left;padding:10px;border-bottom:1px solid #e5e7eb;z-index:1}.si-v31-alert-table td{padding:9px 10px;border-bottom:1px solid #eef2f7;vertical-align:middle}.si-v31-alert-table tr:hover{background:#f8fbff}.si-v31-alert-table .num{text-align:right;font-weight:900}.si-v31-actions{display:flex;gap:6px;justify-content:flex-end;flex-wrap:wrap}.si-v31-btn{border:0;border-radius:9px;padding:7px 9px;font-size:11px;font-weight:900;cursor:pointer}.si-v31-btn.primary{background:#2563eb;color:#fff}.si-v31-btn.warn{background:#f59e0b;color:#fff}.si-v31-btn.ghost{background:#eef2ff;color:#3730a3}.si-v31-tabs{display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-bottom:12px}.si-v31-tabs button{border:1px solid #e2e8f0;border-radius:999px;background:#fff;padding:8px 12px;font-weight:900;font-size:12px;color:#475569}.si-v31-tabs button.active{background:#2563eb;color:#fff;border-color:#2563eb}.si-v31-note{font-size:11px;color:#64748b;margin:4px 0 10px;text-align:center}.si-v31-checkbox{display:flex!important;align-items:center!important;gap:8px!important;margin-top:10px!important;font-size:13px!important;color:#334155!important}
    `;
  ...
  window.si_renderKpi=function(){
    ...
    if(grid){
      ...
      grid.insertAdjacentHTML('beforeend', `<div id="siV31KpiPatch" style="display:contents"><div class="sp-kpi"><div class="ico" style="background:#fee2e2"><i class="fas fa-calendar-times" style="color:#dc2626"></i></div><div class="label">Check Due</div><div class="val" style="color:#dc2626">${num(k.accCheckDue||0)}</div></div><div class="sp-kpi"><div class="ico" style="background:#fef3c7"><i class="fas fa-clock" style="color:#d97706"></i></div><div class="label">Due Soon</div><div class="val" style="color:#d97706">${num(k.accCheckSoon||0)}</div></div><div class="sp-kpi"><div class="ico" style="background:#e0f2fe"><i class="fas fa-repeat" style="color:#0284c7"></i></div><div class="label">M / Q / H</div><div class="val" style="color:#0284c7;font-size:18px">${num(k.accMonthly||0)} / ${num(k.accQuarterly||0)} / ${num(k.accHalfYearly||0)}</div></div></div>`);
    }
  };
...
(function(){
  ...
  function style(){
    ...
    st.textContent=`
      ...
      .si-v32-tabs{display:flex;gap:8px;flex-wrap:wrap;justify-content:center;margin-bottom:12px}.si-v32-tabs button{border:1px solid #dbeafe;background:#fff;color:#334155;border-radius:999px;padding:8px 12px;font-size:12px;font-weight:900;cursor:pointer}.si-v32-tabs button.active{background:#2563eb;color:#fff;border-color:#2563eb}
      .si-v32-toolbar{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:8px 0 10px;flex-wrap:wrap}.si-v32-toolbar-left{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.si-v32-btn{border:0;border-radius:9px;padding:7px 10px;font-weight:900;font-size:11px;cursor:pointer}.si-v32-btn.primary{background:#2563eb;color:#fff}.si-v32-btn.warn{background:#f59e0b;color:#fff}.si-v32-btn.soft{background:#eff6ff;color:#1d4ed8}.si-v32-btn.bad{background:#fee2e2;color:#b91c1c}.si-v32-btn.ok{background:#dcfce7;color:#166534}.si-v32-btn.gray{background:#f1f5f9;color:#334155}
      .si-v32-table-wrap{max-height:540px;overflow:auto;border:1px solid #e2e8f0;border-radius:14px;background:white}.si-v32-table{width:100%;border-collapse:separate;border-spacing:0;font-size:12px}.si-v32-table th{position:sticky;top:0;background:#f8fafc;color:#334155;text-align:left;padding:10px 8px;border-bottom:1px solid #e2e8f0;z-index:2}.si-v32-table td{padding:9px 8px;border-bottom:1px solid #edf2f7;vertical-align:middle}.si-v32-table tr:hover td{background:#f8fbff}.si-v32-table .num{text-align:right;font-variant-numeric:tabular-nums}.si-v32-table .chk{text-align:center;width:34px}.si-v32-qty{width:74px!important;border:1px solid #cbd5e1!important;border-radius:8px!important;padding:6px!important;text-align:center!important;font-weight:900!important}.si-v32-chip{display:inline-flex;align-items:center;gap:4px;border-radius:999px;padding:4px 8px;font-size:10px;font-weight:900;margin:2px;border:1px solid}.si-v32-chip.neutral{background:#f8fafc;color:#334155;border-color:#e2e8f0}.si-v32-chip.danger{background:#fee2e2;color:#b91c1c;border-color:#fecaca}.si-v32-chip.warn{background:#fff7ed;color:#c2410c;border-color:#fed7aa}.si-v32-chip.ok{background:#dcfce7;color:#166534;border-color:#bbf7d0}.si-v32-empty{padding:28px;text-align:center;color:#64748b}.si-v32-kpi-action .val{font-size:24px!important}.swal2-popup .si-v32-table-wrap{text-align:left}
    `;
    document.head.appendChild(st);
  }
  ...
  window.si_renderKpi = function(){
    ...
    if(grid){
      ...
      grid.insertAdjacentHTML('beforeend', `<div id="siV32KpiPatch" style="display:contents"><div class="sp-kpi si-v32-kpi-action"><div class="ico" style="background:#fee2e2"><i class="fas fa-triangle-exclamation" style="color:#dc2626"></i></div><div class="label">Action Required</div><div class="val" style="color:#dc2626">${num(actionCount)}</div></div><div class="sp-kpi"><div class="ico" style="background:#fef3c7"><i class="fas fa-clock" style="color:#d97706"></i></div><div class="label">Due Soon</div><div class="val" style="color:#d97706">${num((alerts.checkSoon||[]).length || k.accCheckSoon || 0)}</div></div></div>`);
    }
  };
