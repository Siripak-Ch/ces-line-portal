...
function renderRevenueTables(list) {
  const renderTableSafe = function (id, key, hexColor) {
    ...
    tbody.innerHTML = list.map(function (d) {
      ...
      if (item.pct >= 80) pctColor = 'text-emerald-500';
      else if (item.pct >= 50) pctColor = 'text-amber-500';
...
function openRevenueEditModal(team) {
  ...
  header.style.backgroundColor = colors[currentEditTeam] || '#1f2937';
...
function openBulkEditModal() {
  ...
  tbody.innerHTML = revenueCache.monthly.map(function (m) {
    return '<tr class="hover:bg-gray-50/50 transition-colors">' +
      ...
      '<td class="p-2"><input type="text" class="bulk-edit-input text-emerald-700" data-month="' + m.month + '" data-field="ehs-t" value="' + Number(m.ehs.t || 0).toLocaleString() + '" onblur="handleInputComma(this)"></td>' +
      '<td class="p-2"><input type="text" class="bulk-edit-input text-emerald-900" data-month="' + m.month + '" data-field="ehs-a" value="' + Number(m.ehs.a || 0).toLocaleString() + '" onblur="handleInputComma(this)"></td>' +
    '</tr>';
  }).join('');
  modal.classList.remove('hidden');
}
  ...
  revenueChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        y: { beginAtZero: true, grid: { color: '#f3f4f6' }, ticks: { callback: function (v) { return (v / 1000000).toFixed(0) + 'M'; } } }
      },
      plugins: {
