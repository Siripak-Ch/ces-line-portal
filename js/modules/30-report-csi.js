...
const R_COLORS = { 'MED': '#0046a3', 'LAB': '#006cff', 'EHS': '#0fc1a1' };
  ...
  ['All','MED','LAB','EHS'].forEach(t => {
    ...
    if(btn) {
      if(rFilters.team === t) {
        ...
        btn.className = "px-4 py-2 rounded-lg text-xs font-bold text-white bg-emerald-600 shadow-md transform scale-105 transition-all";
      } else {
        // สถานะปกติ (เขียวอ่อน)
        btn.className = "px-4 py-2 rounded-lg text-xs font-bold text-emerald-600 bg-emerald-50 hover:bg-emerald-600 hover:text-white transition-all";
      }
    }
  });
  updateReportUI();
...
function renderCRMTable() {
  ...
  tbody.innerHTML = displayData.map(d => {
    ...
    const statusBtn = ticket ? '<span class="bg-green-100 text-green-700 px-3 py-1 rounded-full font-bold border border-green-200">Closed</span>' : '<span class="bg-red-100 text-red-700 px-3 py-1 rounded-full font-bold border border-red-200 animate-pulse">On-Going</span>';
...
function renderCSITable() {
    ...
    tbody.innerHTML = data.map(d => {
        ...
        let satStyle = sat < 3 ? 'text-red-500 font-bold' : 'text-emerald-600 font-bold';
        ...
        return `
            <tr class="hover:bg-slate-50 transition-colors border-b">
                ...
                <td class="p-3 text-[10px] font-bold text-emerald-600">${R_TEAM_MAP[d.team] || d.team}</td>
    ...
    html: `
      <div class="text-left text-sm space-y-3">
        ...
        <div class="bg-green-50 p-3 rounded-lg border border-green-200">
          <p class="font-bold text-green-500 uppercase text-[10px]">Solution:</p>
          <p class="text-green-800">${t.solution}</p>
        </div>
