...
// ==========================================
    ...
    function updateCalendarUI() {
        ...
        if(titleEl) titleEl.innerHTML = `<span class="text-orange-600">${monthNames[monthIndex]}</span> <span class="text-gray-400 font-light">|</span> ${year}`;
        ...
        ['all', 'med', 'lab', 'ehs','mgt'].forEach(id => {
            ...
            if (btn) {
                if (id === service.toLowerCase()) {
                    ...
                    btn.className = "px-3 py-1 text-xs font-bold rounded bg-orange-100 text-orange-700 shadow-sm ring-1 ring-orange-300 transition-all";
                } else {
                    // ปรับสีปุ่มปกติ (Hover เป็นสีส้มเข้ม)
                    btn.className = "px-3 py-1 text-xs font-bold rounded hover:bg-orange-500 hover:text-white transition-all text-gray-500";
                }
            }
        });
        updateCalendarUI();
    ...
    function renderCapacityBars(manDays, weekdays) {
        ...
        teams.forEach(t => {
            ...
            const barColorClass = isOver ? 'bg-rose-500' : t.bg;
            const textClass = isOver ? 'text-rose-600' : 'text-gray-800';
            const icon = isOver ? '<i class="fas fa-exclamation-circle text-rose-500 ml-1"></i>' : '';
    ...
    function renderJobTable(list) {
        ...
        let html = '';
        list.forEach(item => {
            let teamClass = "text-[#0fc1a1] bg-[#0fc1a1]/5 border-[#0fc1a1]/20"; 
            ...
            if(item.team === 'MGT') teamClass = "text-[#8b5cf6] bg-[#8b5cf6]/5 border-[#8b5cf6]/20";
            html += `
                <tr class="bg-white border-b hover:bg-orange-50/50 transition-colors group">
                    ...
                    <td class="px-4 py-3 align-top">
                        <div class="font-bold text-gray-800 text-sm mb-0.5 group-hover:text-orange-600 transition-colors">${item.title}</div>
    ...
    function openJobTracker2025() {
        ...
        let containerHtml = `
            <div class="text-left font-prompt">
                <div class="mb-6 space-y-4 bg-orange-50/50 p-4 rounded-2xl border border-orange-100 shadow-sm">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex bg-white p-1 rounded-xl border border-orange-200 shadow-sm">
                            <button onclick="filterTracker('ALL')" id="btn-tracker-ALL" class="tracker-btn active px-4 py-1.5 rounded-lg text-xs font-bold transition-all bg-orange-500 text-white shadow-sm">All</button>
                            <button onclick="filterTracker('MED')" id="btn-tracker-MED" class="tracker-btn px-4 py-1.5 rounded-lg text-xs font-bold transition-all text-gray-500 hover:bg-orange-100 hover:text-orange-600">MED</button>
                            <button onclick="filterTracker('LAB')" id="btn-tracker-LAB" class="tracker-btn px-4 py-1.5 rounded-lg text-xs font-bold transition-all text-gray-500 hover:bg-orange-100 hover:text-orange-600">LAB</button>
                            <button onclick="filterTracker('EHS')" id="btn-tracker-EHS" class="tracker-btn px-4 py-1.5 rounded-lg text-xs font-bold transition-all text-gray-500 hover:bg-orange-100 hover:text-orange-600">EHS</button>
                        </div>
                        <div class="flex flex-wrap items-center gap-2">
                            <select id="tracker-status-select" onchange="filterTracker()" class="bg-white border border-orange-200 rounded-lg px-2 py-1.5 text-xs font-bold outline-none focus:ring-2 focus:ring-orange-200 cursor-pointer shadow-sm">
                                <option value="ALL">All Status</option>
                                <option value="Matched">Matched</option>
                                <option value="Pending">Pending</option>
                            </select>
                            <select id="tracker-month-select" onchange="filterTracker()" class="bg-white border border-orange-200 rounded-lg px-2 py-1.5 text-xs font-bold outline-none focus:ring-2 focus:ring-orange-200 cursor-pointer shadow-sm">
                            ...
                            <button onclick="exportTrackerToCSV()" class="bg-orange-500 hover:bg-orange-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm">
                    ...
                    <table class="w-full text-xs border-collapse">
                        <thead class="bg-gray-50 sticky top-0 z-10 border-b border-gray-100 shadow-sm">
                            <tr>
                                ...
                                <th class="p-4 text-center font-bold text-green-600 uppercase tracking-wider w-[40%]"><i class="fas fa-calendar-check"></i> 2026 Status</th>
                            </tr>
                        </thead>
        ...
        Swal.fire({
            title: '<div class="flex items-center gap-3"><i class="fas fa-tasks text-orange-500"></i> Plan Comparision: 2025 vs 2026</div>',
            html: containerHtml,
            width: '1050px',
    ...
    function filterTracker(team) {
        ...
        document.querySelectorAll('.tracker-btn').forEach(btn => {
            btn.classList.remove('bg-orange-500', 'text-white', 'shadow-sm', 'active');
        ...
        if (activeBtn) {
            ...
            activeBtn.classList.add('bg-orange-500', 'text-white', 'shadow-sm', 'active');
        }
        ...
        tbody.innerHTML = filtered.map(r => {
            let teamBadge = `<span class="px-2.5 py-1.5 rounded-md text-[10px] font-black bg-emerald-50 text-emerald-600 border border-emerald-100 shadow-sm">${r.team}</span>`;
            ...
            if (r.status === 'Matched') {
                rightSideUI = `
                    <a href="${calLink26}" target="_blank" class="block bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl p-3 w-full shadow-sm text-left hover:shadow-md hover:border-green-300 transition-all cursor-pointer group/link">
                        <div class="flex items-center gap-1.5 mb-2">
                            <i class="fas fa-check-circle text-green-500 text-sm"></i>
                            <span class="text-[10px] font-extrabold text-green-700 uppercase tracking-widest bg-white px-2 py-0.5 rounded shadow-sm">Matched in 2026</span>
                        </div>
                        <div class="text-sm font-bold text-gray-800 leading-snug mb-2 group-hover/link:text-green-700">${r.matchTitle}</div>
                        <div class="flex flex-col gap-1.5">
                            <div class="text-xs text-gray-600 flex items-center gap-2"><i class="far fa-calendar-alt w-3 text-center text-green-600"></i> <span class="font-medium bg-white px-2 py-0.5 rounded border border-green-100">${r.dateRange26}</span></div>
                        </div>
                    </a>
                `;
            } else {
                rightSideUI = `
                    <div class="bg-amber-50/60 border-2 border-dashed border-amber-200 rounded-xl p-4 w-full h-full flex flex-col items-center justify-center text-center transition-all hover:bg-amber-50">
                        <i class="fas fa-clock text-amber-400 text-2xl mb-2 drop-shadow-sm"></i>
                        <span class="text-xs font-extrabold text-amber-700 uppercase tracking-wide">Pending</span>
                        <span class="text-[10px] text-amber-600 mt-1 font-medium bg-white px-2 py-1 rounded-md shadow-sm border border-amber-100">No similar job found in 2026</span>
                    </div>
                `;
            }

            return `
            <tr class="hover:bg-orange-50/30 border-b border-gray-100 transition-colors group">
                ...
                <td class="p-4 align-middle text-center w-[10%] bg-gray-50/30">
                    <div class="flex justify-center items-center h-full">
                        <div class="bg-white text-gray-300 rounded-full w-8 h-8 flex items-center justify-center shadow-sm border border-gray-100 group-hover:text-orange-400 group-hover:border-orange-200 transition-all">
