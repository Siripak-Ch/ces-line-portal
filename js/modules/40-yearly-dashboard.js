        ...
        yearlyTrendChartInstance = new Chart(ctxT, {
            ...
            data: {
                ...
                datasets: [
                    ...
                    { 
                        label: 'Target (MED)', data: targets.map(t=>t.med), backgroundColor: targetColor, hoverBackgroundColor: '#d1d5db',
                        order: 2, borderRadius: 4, datalabels: { display: false }, ...barWidthSettings 
                    },
                    { 
                        label: 'Target (LAB)', data: targets.map(t=>t.lab), backgroundColor: targetColor, hoverBackgroundColor: '#d1d5db',
                        order: 2, borderRadius: 4, datalabels: { display: false }, ...barWidthSettings
                    },
                    { 
                        label: 'Target (EHS)', data: targets.map(t=>t.ehs), backgroundColor: targetColor, hoverBackgroundColor: '#d1d5db',
                        order: 2, borderRadius: 4, datalabels: { display: false }, ...barWidthSettings
                    },
    ...
    function renderYearlyTable(data) {
        ...
        tbody.innerHTML = sorted.map(d => `
            <tr onclick="goToCalendarMonth(${d.month}, ${d.year})" class="hover:bg-gray-50 transition-colors cursor-pointer">
                ...
                <td class="text-right font-mono text-[#004aad] font-bold">${d.med}</td>
  ...
  window.renderYearlyTable = function (data) {
    ...
    tbody.innerHTML = rows.map(d => `
      <tr onclick="goToCalendarMonth(${yNum(d.month)}, ${yNum(d.year)})" class="hover:bg-gray-50 transition-colors cursor-pointer">
        ...
        <td class="text-right font-mono text-[#004aad] font-bold">${yNum(d.med)}</td>
