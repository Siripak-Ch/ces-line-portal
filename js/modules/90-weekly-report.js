...
function switchWeeklyView(mode) {
    ...
    const activeFormCls = "flex-1 md:flex-none px-5 py-2.5 rounded-lg text-sm font-bold shadow-sm bg-white text-yellow-600 transition-all border border-transparent";
    const inactiveCls = "flex-1 md:flex-none px-5 py-2.5 rounded-lg text-sm font-bold text-gray-500 hover:text-yellow-600 transition-all";
...
function setReportProgress(val) {
    ...
    [20, 40, 60, 80, 100].forEach(pt => {
        ...
        if(btn) {
            if(parseInt(val) === pt) {
                btn.className = "prog-btn flex-1 py-2 text-xs font-bold rounded-lg transition-all shadow-md bg-yellow-500 text-white border-transparent";
            } else {
        ...
        <div class="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all">
            <div class="flex-1 pr-4">
                <div class="flex items-center gap-2 mb-1">
                    <span class="text-[9px] font-black bg-yellow-50 text-yellow-700 px-2 py-0.5 rounded uppercase tracking-wider">${d.mainTeam} / ${d.subTeam}</span>
                    <span class="text-[9px] font-bold ${d.status==='Finish'?'text-green-600':'text-yellow-600'} border px-1.5 py-0.5 rounded">${d.status}</span>
                </div>
...
function filterDash(team) {
    ...
    ['ALL', 'MED', 'LAB', 'EHS'].forEach(t => {
        ...
        if(btn) {
            btn.className = "px-5 py-2 rounded-lg text-sm font-bold bg-white text-gray-500 border border-transparent hover:text-yellow-600 transition-all whitespace-nowrap"; 
...
function filterStatusDash(status) {
    ...
    ['ALL', 'On Process', 'Finish'].forEach(s => {
        ...
        if(btn) {
            btn.className = "px-5 py-2 rounded-lg text-sm font-bold bg-white text-gray-500 border border-transparent hover:text-yellow-500 transition-all whitespace-nowrap";
            let icon = s==='On Process'?'<i class="fas fa-spinner text-yellow-500 mr-1.5"></i>':s==='Finish'?'<i class="fas fa-check text-green-500 mr-1.5"></i>':'';
            if(s === status) btn.className = `px-5 py-2 rounded-lg text-sm font-bold text-white shadow-sm transition-all whitespace-nowrap ${s==='Finish'?'bg-green-500':s==='On Process'?'bg-yellow-500':'bg-gray-800'}`;
            if(s === status && s !== 'ALL') icon = icon.replace('text-yellow-500', 'text-white').replace('text-green-500', 'text-white');
...
function createPhotoCard(r) {
    const colors = {'MED':{c:'bg-med',t:'text-med',bg:'bg-blue-50'}, 'LAB':{c:'bg-lab',t:'text-lab',bg:'bg-cyan-50'}, 'EHS':{c:'bg-ehs',t:'text-ehs',bg:'bg-emerald-50'}};
    ...
    const barColor = isFin ? '#22c55e' : (r.team === 'MED' ? '#004aad' : (r.team === 'LAB' ? '#19a7ce' : '#0fc1a1'));
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col group relative" data-id="${r.rowId}">
        <div class="absolute top-2 right-2 flex gap-1 z-20">
            <button onclick="editCard('${r.rowId}')" class="w-8 h-8 bg-white/95 rounded-full shadow text-yellow-500 hover:text-yellow-700 flex items-center justify-center backdrop-blur-sm transition-colors"><i class="fas fa-pen text-xs"></i></button>
        ...
        <div class="p-4 flex-1 flex flex-col">
            <div class="flex justify-between items-start mb-3">
                ...
                <span class="text-[9px] font-bold px-2 py-0.5 rounded-full border shadow-sm ${isFin?'bg-green-100 text-green-700 border-green-200':'bg-yellow-50 text-yellow-700 border-yellow-200'}">${r.status}</span>
            </div>
            <h4 class="font-bold text-gray-800 text-sm mb-1 leading-snug line-clamp-2 cursor-pointer hover:text-yellow-600" onclick="viewDetailByRowId('${r.rowId}')">${r.job}</h4>
            ...
            <div class="mt-auto pt-3 mt-3 border-t border-dashed border-gray-100">
                <div class="mb-2">
                    <div class="flex justify-between text-[9px] mb-1">
                        ...
                        <span class="font-bold ${isFin?'text-green-600':c.t}">${devDisplay}</span>
                    </div>
                ...
                <div class="mb-1">
                    <div class="flex justify-between text-[9px] mb-1">
                        ...
                        <span class="font-bold ${isFin?'text-green-600':c.t}">${rptDisplay}</span>
                    </div>
                    <div class="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
    ...
    Swal.fire({
        ...
        <div class="space-y-4 text-left font-sans mt-2">
            <div class="flex justify-between items-center text-xs pb-3 border-b border-gray-100">
                ...
                <span class="px-2.5 py-0.5 rounded-full ${r.status==='Finish'?'bg-green-100 text-green-700 border-green-200':'bg-yellow-50 text-yellow-700 border-yellow-200'} font-bold border">${r.status}</span>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 text-xs">
                ...
                <div><span class="text-gray-400 font-bold block mb-1">Last Update</span><span class="text-gray-700 font-bold bg-gray-100 px-2 py-0.5 rounded-md inline-block"><i class="fas fa-history text-green-400 mr-1"></i>${r.ts ? r.ts.split(' ')[0] : '-'}</span></div>
                ...
                <div class="col-span-2"><span class="text-gray-400 font-bold block mb-1.5">Team Members</span><div class="flex flex-wrap gap-1.5">${(r.members||'-').split(',').map(m=>`<span class="bg-yellow-50 border border-yellow-100 text-yellow-700 px-2 py-0.5 rounded-md font-bold">${m.trim()}</span>`).join('')}</div></div>
            </div>
            ...
            <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm mt-3">
                ...
                <div>
                    <div class="flex justify-between text-[10px] mb-1.5">
                        <span class="text-gray-500 font-bold"><i class="fas fa-chart-line text-orange-500 mr-1"></i> Report Progress</span>
                        <span class="font-bold text-orange-500">${rptPct}%</span>
                    </div>
                    <div class="w-full bg-gray-100 h-2 rounded-full overflow-hidden"><div class="bg-orange-400 h-full rounded-full transition-all duration-1000" style="width: ${rptPct}%"></div></div>
                </div>
            </div>
...
function deleteCard(rowId) { Swal.fire({title:'Delete?', text:"Undone action", icon:'warning', showCancelButton:true, confirmButtonColor:'#ef4444', confirmButtonText:'Delete'}).then((r)=>{ if(r.isConfirmed) google.script.run.withSuccessHandler(loadWeeklyDashboard).deleteWeeklyReport(rowId); }); }
...
function renderSelectedMembers(){ document.getElementById('wk-member-tags').innerHTML=wkSelectedMembers.map(m=>`<span class="inline-flex items-center gap-1 px-3 py-1 rounded-lg bg-yellow-50 text-yellow-700 text-xs font-bold border border-yellow-100 shadow-sm">${m}<button type="button" onclick="removeMember('${m}')" class="hover:text-red-500 ml-1"><i class="fas fa-times"></i></button></span>`).join(''); }
...
function openCloseJobModal() {
    try {
        ...
        if (container) {
            container.innerHTML = '<div class="text-center py-10"><i class="fas fa-spinner fa-spin text-3xl text-yellow-500"></i><p class="text-sm font-bold mt-3 text-gray-500">Preparing data...</p></div>';
        }
    ...
    try {
        ...
        let filterTabs = `
            <div class="flex gap-2 mb-4 bg-gray-100 p-1.5 rounded-xl sticky top-0 z-10 shadow-sm border border-gray-200">
                ${['ALL', 'MED', 'LAB', 'EHS'].map(t => `<button type="button" onclick="wkActiveCloseFilter='${t}'; renderCloseJobModal();" class="flex-1 py-2 text-xs font-bold rounded-lg transition-all ${wkActiveCloseFilter === t ? 'bg-white shadow-sm text-yellow-600' : 'text-gray-500 hover:bg-gray-200'}">${t}</button>`).join('')}
            </div>
        `;
        ...
        container.innerHTML = filterTabs + '<div class="space-y-4">' + filtered.map(j => {
            ...
            let rptCompletedHtml = `<span class="text-[9px] text-orange-600 font-bold"><i class="fas fa-check-circle mr-1"></i>Will update to 100%</span>`;
            ...
            return `
            <div class="flex flex-col p-4 bg-white rounded-2xl border border-gray-200 hover:border-yellow-300 shadow-sm job-close-row transition-all" data-rowid="${j.rowId}">
                ...
                <div class="mt-3 grid grid-cols-2 gap-3">
                    <label class="flex items-center gap-2 p-2.5 rounded-xl border ${isDevDone ? 'bg-green-50 border-green-200 cursor-not-allowed' : 'bg-white border-gray-200 hover:border-blue-400 cursor-pointer'} transition-all">
                        ...
                        <div class="flex flex-col flex-1 data-container" data-original-html='${devOriginalHtml}' data-completed-html='${devCompletedHtml}'>
                            <span class="text-[11px] font-black title-span ${isDevDone ? 'text-green-700' : 'text-gray-700'}">Device 100%</span>
                            <div class="status-content mt-0.5">
                                ${isDevDone ? `<span class="text-[9px] text-green-500 font-bold"><i class="fas fa-check mr-1"></i>Completed</span>` : devOriginalHtml}
                            </div>
                        </div>
                    </label>
                    
                    <label class="flex items-center gap-2 p-2.5 rounded-xl border ${isRepDone ? 'bg-green-50 border-green-200 cursor-not-allowed' : 'bg-white border-gray-200 hover:border-orange-400 cursor-pointer'} transition-all">
                        <input type="checkbox" class="chk-rep w-5 h-5 text-orange-500 border-gray-300 rounded focus:ring-orange-500" ${isRepDone ? 'checked disabled' : ''} onchange="toggleCloseCheckbox(this, 'rep')">
                        <div class="flex flex-col flex-1 data-container" data-original-html='${rptOriginalHtml}' data-completed-html='${rptCompletedHtml}'>
                            <span class="text-[11px] font-black title-span ${isRepDone ? 'text-green-700' : 'text-gray-700'}">Report 100%</span>
                            <div class="status-content mt-0.5">
                                ${isRepDone ? `<span class="text-[9px] text-green-500 font-bold"><i class="fas fa-check mr-1"></i>Completed</span>` : rptOriginalHtml}
                            </div>
                        </div>
                    </label>
                </div>
    ...
    try {
        ...
        if (cb.checked) {
            ...
            if (type === 'dev') {
                ...
                titleSpan.classList.remove('text-gray-700');
                titleSpan.classList.add('text-orange-700');
            }
            ...
            label.classList.remove('bg-blue-50', 'border-blue-400', 'bg-orange-50', 'border-orange-400');
            titleSpan.classList.remove('text-blue-700', 'text-orange-700');
...
function resetToNewWeeklyForm() {
    ...
    if (btnDraft) {
        btnDraft.className = 'flex-1 md:flex-none px-6 py-3 rounded-xl font-bold text-yellow-600 bg-yellow-50 hover:bg-yellow-100 transition-all shadow-sm border border-yellow-100 flex items-center justify-center gap-2';
    ...
    let btnSubmit = document.getElementById('btn-wk-submit-all');
    if (btnSubmit) {
        btnSubmit.className = 'flex-1 px-8 py-3 rounded-xl font-black text-white bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-600 hover:to-amber-600 transition-all shadow-md transform hover:-translate-y-0.5 flex items-center justify-center gap-2';
